...
import { POST } from "../../app/(api)/update_route_saved+api";
...
describe("POST is_saved update API", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    process.env.DATABASE_URL = "test-database-url";
  });

  it("updates the route with is_saved set to true", async () => {
    // Arrange: simulate a valid SQL update.
    const fakeResult = [{ id: 1, clerk_id: "123", difficulty: "medium", is_saved: true }];
    const sqlMock = jest.fn().mockResolvedValue(fakeResult);
    (neon as jest.Mock).mockReturnValue(sqlMock);

    const requestBody = { clerkId: "123", difficulty: "medium", saved: true };
    ...
    expect(staticSQL).toMatch(/SET is_saved = true/);
    ...
    // The tagged template contains 4 substitutions:
    // 1. clerkId for the main WHERE clause,
    // 2. difficulty for the main WHERE clause,
    // 3. clerkId for the subquery,
    // 4. difficulty for the subquery.
    expect(substitutions.length).toBe(4);
    expect(substitutions[0]).toBe("123");
    expect(substitutions[1]).toBe("medium");
    expect(substitutions[2]).toBe("123");
    expect(substitutions[3]).toBe("medium");
  });
  ...
  it("returns 500 and logs error when an exception occurs", async () => {
    ...
    // Spy on console.log to check error logging.
